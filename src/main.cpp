...
// playground -- lower start difficulty
...
ForkInEffect (Fork type, unsigned nHeight)
{
  switch (type)
    {
    // playground -- custom fork heights
    case FORK_POISON:
      return nHeight >= (fTestNet ? 100 : 255000); // 190000 : 255000);

    case FORK_CARRYINGCAP:
      return nHeight >= (fTestNet ? 110 : 500000); // 200000 : 500000

    case FORK_LESSHEARTS:
      return nHeight >= (fTestNet ? 120 : 590000); // 240000 : 590000
...
int CMerkleTx::GetBlocksToMaturity() const
{
    ...
    if (hashBlock == hashGenesisBlock)   // Genesis block is immediately spendable
        return INT_MAX; // playground -- Genesis block not immediately spendable, was: return 0;
    ...
    if (!IsCoinBase())
    {
        ...
        for (int i = 0; i < vin.size(); i++)
        {
            ...
            if (!IsGameTx ())
            {
                ...
                // playground -- Genesis block not immediately spendable
                if (txo.height == 0)
...
bool LoadBlockIndex(bool fAllowNew)
{
    if (fTestNet)
    {
        ...
        pchMessageStart[2] = 0xb7; // playground -- different message start, was 0xb5
...
char pchMessageStart[4] = { 0xf9, 0xbe, 0xb6, 0xd9 }; // playground -- different message start, was 0xf9, 0xbe, 0xb4, 0xd9
