...
GetNameCoinAmount (unsigned nHeight, bool frontEnd)
{
  ...
  // playground -- custom locked coin amount
  return 20 * COIN;
...
Value name_update(const Array& params, bool fHelp)
{
    ...
    else
    {
        if ((s0 == "devmode") || (s0 == "d"))
        {
            if ((s[0] < '3') || (s[0] > '9'))
            {
                // hack -- client only debug text
                //               AI                 not used           not used
                int d = s[0] == 'a' ? 2 : (s[0] == 'f' ? 3 : (s[0] == 't' ? 4 : s[0] - '0'));
                if ((d >= 0) && (d <= 4))
                {
                    Displaycache_devmode = d;
                    return d;
                }

                s[0] = '0'; // any key to switch off (network-wide)
            }

            if ((Displaycache_devmode_npcname.length() < 1) || (!fTestNet))
            {
                throw runtime_error(
                        "devmode: cannot send message to " + Displaycache_devmode_npcname);
            }

            s0 = Displaycache_devmode_npcname;
        }
        ...
        if (s[0] != '{')
        {
            ...
            if ((l0 > 2) && (s0[l0-2] == '.'))
            {
                n0 = (int)s0[l0-1] - (int)'0';
                s0.erase(l0-2, 2);
            }
            else if ((l0 > 3) && (s0[l0-3] == '.'))
            {
                n0 = 10 * ((int)s0[l0-2] - (int)'0') + (int)s0[l0-1] - (int)'0';
                s0.erase(l0-3, 3);
            }

            // store some chars per hunter (used for devmode)
            int l = s.length();
            if (l > ALTNAME_LEN_MAX) l = ALTNAME_LEN_MAX;
            char buf[200];
            char buf2[20];
            sprintf (buf, "{\"%d\":{\"wp\":[0,21,", n0);
            for (int i = 0; i < l; i+=2)
            {
                int n = s[i];
                int n2 = i<l-1 ? s[i+1] : 0; // odd number of chars?
                if (!(ALTNAME_ASCII_OK(n))) n = 0;
                if (!(ALTNAME_ASCII_OK(n2))) n2 = 0;
                if (i<l-2) sprintf (buf2, "%d,%d,", n, n2);
                else sprintf (buf2, "%d,%d]}}", n, n2);
                strcat(buf, buf2);
            }
            s2.assign(buf);
        }
    }
    vector<unsigned char> vchName = vchFromString(s0);
    vector<unsigned char> vchValue = vchFromString(s2);
